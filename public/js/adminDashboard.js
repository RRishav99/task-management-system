document.addEventListener('DOMContentLoaded', function () {
  ...
  searchTaskLink.addEventListener('click', 
  function(){
    if(!searchButtonClicked){ searchTasks()}});
  ...
  function displayTasks(tasks) {
    ...
    // Add table headers
    const headers = ['taskId','Task Title', 'TASK DESCRIPTION', 'PRIORITY', 'ASSIGNED','status','createdAt','updatedAt','messages'];
    const headerRow = document.createElement('tr');
    headers.forEach(header => {
      const th = document.createElement('th');
      if (header=='taskId'){
      th.innerHTML = header + `<span class="icon-arrow" onclick="sTable('${0}')">&UpArrow;</span>` ;
    ...
    // Add task rows
    const headers1 = ['taskId','Task_Title', 'TASK_DESCRIPTION', 'PRIORITY_LEVEL', 'ASSIGNED_TO_USER','status','createdAt','updatedAt','messages'];
  ...
  //function to concert time format
    ...
    fetch(url)
      ...
      .then((data) => {
        ...
        for (i = 0; i < length; i++) {
          ...
          content += `<td>${data[i].createdAt}</td>`;
          content += `<td>${data[i].updatedAt}</td>`;
        } 
        content += `</select>`;
...
const canceldata = document.getElementById('cancel1');
canceldata.addEventListener('click', cancelUpdateTask);
