...
const JWT_SECRET = 'sdjkfh8923yhjdksbfma@#*(&@*!^#&@bhjb2qiuhesdbhjdsfg839ujkdhfjk';
 ...
 exports.getallusers=(req, res) => {
  ...
  exports.register = async (req, res, next) => {
    const { employeeid, name, email, username, password:plainTextPassword } = req.body;
    console.log(name,email,username,plainTextPassword);
    if(!employeeid||!name || !email || !username || !plainTextPassword)
      return res.status(400).send("Please fill in all the required fields!")
      const password = await bcrypt.hash(plainTextPassword, 10)
    try {
      const userObj = { employeeid, name, email, username,password  };     
